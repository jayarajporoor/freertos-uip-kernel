...
tskTCB *pxNewTCB;
	...
	if( pxNewTCB != NULL )
	{
		...
		{
			pxNewTCB->ws.wait_q = xQueueCreate(1, ( unsigned portLONG ) sizeof( signed portLONG ) );
			pxNewTCB->ws.next = (wait_struct_t*)0UL;
			
			if((xQueueHandle*)0 == pxNewTCB->ws.wait_q)
			{
				/*Couldn't create wait q. Delete stack and TCB*/
...
{
	list->first = (wait_struct_t*)0UL;
	list->last = (wait_struct_t*)0UL;
}

void task_wait_list_enqueue_(wait_list_t *list, wait_struct_t *item)
{
	item->next=item->prev=(wait_struct_t*)0UL; 	

	if((wait_struct_t*)0 == list->first)
	{
		list->first = item;
	}else
	{
...
{
	wait_struct_t *item= (wait_struct_t*)0UL;
	...
	if(item)
	{
		...
		{
			list->first = list->last = (wait_struct_t*)0UL;	
		}else
		{
			list->last = item->prev;
			list->last->next = (wait_struct_t*)0UL;
		}
	}
	return item;
}
