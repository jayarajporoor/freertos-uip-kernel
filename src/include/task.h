...
typedef struct tag_wait_struct
{
	void *wait_q;
	struct tag_wait_struct *next;//next in the list
	struct tag_wait_struct *prev;//prev in the list  
}wait_struct_t;
...
typedef struct tskTaskControlBlock
{
	...
	wait_struct_t 			ws;					/*< used for wait operations on the task*/
} tskTCB;
...
#define CURR_WAIT_STRUCT() (&(((tskTCB*)pxCurrentTCB)->ws))

#define TASK_WAIT(w, pv) xQueueReceive( (w)->wait_q, (signed portCHAR *)pv, portMAX_DELAY ) 
#define TASK_NOTIFY(w, pv) xQueueSend( (w)->wait_q, (signed portCHAR *)pv, 0UL)
#define TASK_NOTIFY_FROM_ISR(w, pv) xQueueSendFromISR( (w)->wait_q, (signed portCHAR *)pv, 0UL)

#define TASK_WAIT_LIST_IS_EMPTY(wl) ((wait_struct_t*)0UL == (wl)->last)

void task_wait_list_init(wait_list_t *list);
void task_wait_list_enqueue_(wait_list_t *list, wait_struct_t *item);
wait_struct_t *task_wait_list_dequeue_(wait_list_t *list);
